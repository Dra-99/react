...
export default function index(props) {
    ...
    for(let i = minPage; i <= maxPage; i++) {
        spanArr.push(<span key={i} className={props.current === i ? `${style.active} ${style.item}` : style.item} onClick={() => {
            changePage(i, props);
        }}>{i}</span>)
    }
    ...
    return (
        <div className={style.container}>
            <span className={props.current === 1 ? `${style.disabled} ${style.item}` : style.item} onClick={() => {
                changePage(1, props)
            }}>首页</span>      
            <span className={props.current === 1 ? `${style.disabled} ${style.item}` : style.item} onClick={() => {
            ...
            {spanArr}
            <span className={props.current === pageTotal ? `${style.disabled} ${style.item}` : style.item} onClick={() => {
                changePage(props.current + 1, props)
            }}>下一页</span>
            <span className={props.current === pageTotal ? `${style.disabled} ${style.item}` : style.item} onClick={() => {
